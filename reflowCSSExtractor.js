var ReflowCSSExtractor = function(csscontent) {
	...
	this.createReport = function(){
		...
		for (color in colors) {
			...
			for (var j = 0; j < 15-rgbString.length; j++){
				rgbString += space;
			}
	...
	this.findColors = function(){
		...
		for(var i = 0; i < sheet.cssRules.length; i++){
	 		...
	 		for(var j = 0; j < sheet.cssRules[i].declarations.length; j++){
		 		...
		 		if (sheet.cssRules[i].declarations[j].property === "color" ||
		 			sheet.cssRules[i].declarations[j].property === "background-color"){
		 			var original = sheet.cssRules[i].declarations[j].valueText;
		 			var color = original;
		 			...
		 			if (color in result){
		 				...
		 				result[color] = new Object();
		 				result[color]["usedAsBackground"] = false;
		 			...
		 			if (sheet.cssRules[i].declarations[j].property === "background-color"){
						result[color]["usedAsBackground"] = true;
					}
	...
	this.findBreakPoints = function (){
		...
		for(var i = 0; i < sheet.cssRules.length; i++){
	 		...
	 		for(var j = 0; j < sheet.cssRules[i].media.length; j++){
	...
	this.priceIsRightGetColorName = function(hexColor) {
		...
		for (var color in colorArray){
			...
			var bc_g = bc_rgb.g;
			var bc_b = bc_rgb.b;	
			differenceArray.push(Math.sqrt((c_r-bc_r)*(c_r-bc_r)+(c_g-bc_g)*(c_g-bc_g)+(c_b-bc_b)*(c_b-bc_b)));
		}
